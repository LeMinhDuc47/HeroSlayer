...
void PlayPK::Load() {
	TextureManager::GetInstance()->Load("player1", "assets/ModePK/" + p1_path);
	TextureManager::GetInstance()->Load("player1_skill", "assets/ModePK/" + p1_skill_path);

	TextureManager::GetInstance()->Load("player2", "assets/ModePK/" + p2_path);
	TextureManager::GetInstance()->Load("player2_skill", "assets/ModePK/" + p2_skill_path);

	TextureManager::GetInstance()->Load("P1HPMN", "assets/hp + mana/mau + mana.png");
	TextureManager::GetInstance()->Load("P1HP", "assets/ModePK/mau.png");
	TextureManager::GetInstance()->Load("P1MN", "assets/ModePK/mana.png");

	TextureManager::GetInstance()->Load("P2HPMN", "assets/ModePK/mau + mana.png");
	TextureManager::GetInstance()->Load("P2HP", "assets/ModePK/mau.png");
	TextureManager::GetInstance()->Load("P2MN", "assets/ModePK/mana.png");

	TextureManager::GetInstance()->Load("muiten1", "assets/ModePK/chon1.png");
	TextureManager::GetInstance()->Load("muiten2", "assets/ModePK/chon2.png");
}
...
void PlayPK::Luu() {
	if (Engine::GetInstance()->GetMenu()->GetPK()) {
		ofstream out("assets/PK.txt");
		if (out.is_open()) {
			out << p1_Transform->X <<endl << p1_Transform->Y << endl;
			out << p1_HP << endl << p1_MN << endl << p1_dame << endl;
			out << p1_IsSkill << endl << p1_time_skill << endl;
			out << p1_skill_frame <<endl << p1_attack_frame << endl;
			out << p1_path <<endl << p1_skill_path << endl;
			out << p1_skill_width << endl << p1_skill_height << endl;

			out << p2_Transform->X << endl << p2_Transform->Y << endl;
			out << p2_HP << endl << p2_MN <<endl << p2_dame << endl;
			out << p2_IsSkill << endl << p2_time_skill << endl;
			out << p2_skill_frame <<endl << p2_attack_frame << endl;
			out << p2_path << endl << p2_skill_path << endl;
			out << p2_skill_width << endl << p2_skill_height << endl;
		}
		out.close();
	}
		ofstream out2("assets/ModePK/Bought.txt");
		if (out2.is_open()) {
			out2 << PKList[0].Isbuy << endl << PKList[1].Isbuy << endl << PKList[2].Isbuy <<endl;
			out2 << PKList[3].Isbuy << endl << PKList[4].Isbuy << endl << PKList[5].Isbuy << endl;
			out2 << PKList[6].Isbuy << endl << PKList[7].Isbuy << endl << PKList[8].Isbuy << endl;
			out2 << PKList[9].Isbuy << endl;
		}
}

void PlayPK::SetContinue() {
	ifstream in("assets/PK.txt");
	string line;
	int dem = 0;
...
void PlayPK::SetPrevious() {
	ifstream in("assets/ModePK/Bought.txt");
	string line;
	int dem = 0;
	while (getline(in, line)) {
		dem++;
