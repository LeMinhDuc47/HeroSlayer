...
void Boss::Load() {
	TextureManager::GetInstance()->Load("boss", "assets/Boss/" + Frieza[CurrentBoss].Texture);
	TextureManager::GetInstance()->Load("boss_skill", "assets/Boss/" + Frieza[CurrentBoss].skill_Texture);
	TextureManager::GetInstance()->Load("boss_hp", "assets/Boss/HP.png");
}

void Boss::Luu() {
	ofstream out("assets/Pause/boss.txt");
	if (out.is_open()) {
		out << e_Transform->X <<endl << e_Transform->Y <<endl;
		out << e_HP <<endl << e_dame <<endl;
		out << CurrentBoss <<endl << e_time <<endl;
	}
	out.close();
...
void Boss::SetContinue() {
	ifstream in("assets/Pause/boss.txt");
	string line;
	int dem = 0;
