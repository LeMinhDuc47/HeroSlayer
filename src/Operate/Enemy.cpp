	...
	if (Engine::GetInstance()->GetMap1()) {
		...
		if (e_HP < 0 && CurrentEnemy == TDST.size() - 1) {
			...
			if (CollisionHandler::GetInstance()->CheckCollision(box_Enemy, box_Player)) {
				...
				e_IsAttacking = false;
				vector<Point> duongdi = ShortestPath::GetInstance()->cungKhuDat(m_P, e_P);
...
void Enemy::Load() {
	TextureManager::GetInstance()->Load("enemy", "assets/enemy/" + TDST[CurrentEnemy].Texture);
	TextureManager::GetInstance()->Load("enemy_hp", "assets/enemy/HP.png");

}

void Enemy::Luu() {
	ofstream out("assets/Pause/enemy.txt");
	if (out.is_open()) {
		out << e_Transform->X << endl << e_Transform->Y << endl;
		out << e_HP << endl << e_dame << endl;
		out << CurrentEnemy << endl << e_time << endl;
	}
	out.close();
...
void Enemy::SetContinue() {
	ifstream in("assets/Pause/enemy.txt");
	string line;
	int dem = 0;
