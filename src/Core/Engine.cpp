...
bool Engine::Init()
{
    ...
    m_Window = SDL_CreateWindow("Adventure", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SCREEN_WIDTH, SCREEN_HEIGHT, window_flags);
    ...
    SDL_Surface* iconSurface = IMG_Load("assets/menu/icon.png");
    if (!iconSurface) {
    ...
    m_FontManager = new FontManager("assets/Font/Roboto.ttf", 24);
    m_FontManager->loadFont();

    TextureManager::GetInstance()->Load("mui_ten", "assets/maps/chiduong.png");
    ...
    TextureManager::GetInstance()->Load("bg1", "assets/Bg/bg.png");
    TextureManager::GetInstance()->Load("bg2", "assets/Bg/rock.png");
...
void Engine::Update()
{
    ...
    if (gameoverlose || gameoverwin) {
        if (!updatescore) {
            ...
            sort(highscore, highscore + 5, [](int a, int b) {
                return a > b;
                });
            }
            updatescore = true;
...
void Engine::GetPrevious() {
    ifstream in("assets/previous.txt");
    string line;
    int dem = 0;
...
void Engine::Luu() {
    ofstream out("assets/previous.txt");
    out << gold << endl;
    if (out.is_open()) {
        for (int i = 0; i < 5; i++) {
            out << highscore[i] <<endl;
        }
    }
    out.close();
}
